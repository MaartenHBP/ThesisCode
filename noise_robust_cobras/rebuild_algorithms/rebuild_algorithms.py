...
class ClosestRebuild(Rebuilder):
    def rebuild(self, repres, indices, data, indi_super=[], labelled=[]): # werken via kNN, indices bevatten ook de represkes
        if len(labelled) == 0:
            ...
            _, indices = nbrs.kneighbors(data[indices])
            # for i in len(repres): # repres wel nog altijd hun eigen nemen (dit is alleen nodig als twee repres overeenlappen)
            #     indices[indi_super == repres[i]] = i, gaan er nu gewoon van uit dat het klopt
            return indices
...
class SemiCluster(Rebuilder):
    def rebuild(self, repres, indices, data, indi_super=[], labelled=[]):
        ...
        repers_indi = indices[np.in1d(indices,repres)]
        ...
        while True:
            ...
            #     break
            if len(np.unique(newlabels[repers_indi])) != len(repres): # de repres zijn eruit geconvergeerd, beter alternatief
                break
            ...
            for i in len(centers):
                centers[i] = data[indices[labels == i]].mean(axis = 0)
